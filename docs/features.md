...
- **面のテクスチャは同梱素材からも選べる** — 画像ライブラリに加えて `WALL_TEXTURE_IDS` / `TEXTURE_IDS`（`domain/media/texture-catalog`）のパレットを面ごとに開く。選ぶと `DungeonBuildService.registerAsset()` が URL 単位で `ImageStorage` に一度だけ登録して identifier を返す（ランダムダンジョンと同じ経路なので、同じテクスチャを両方から使っても画像は増えない）
